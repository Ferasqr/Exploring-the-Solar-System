...
vec3 BRDF(vec3 V, vec3 N, vec3 L, vec3 Md, vec3 Ms, float gamma) {
	...
	//Lambertian reflection is defined as -> md . max(lx.nx,0)
	vec3 Fdiff = Md * clamp(dot(L, N), 0.0, 1.0);

	//Blinn normMap is -> Ms * clamp(nx.hlx)^gamma
	vec3 hlx = normalize(L+V);
	vec3 Fspec = Ms * clamp(pow(dot(N,hlx),gamma), 0.0, 1.0);
